            ...
            for (std::vector<std::string>::iterator it = curr_sentence.begin(); it != curr_sentence.end(); it++) {
                ...
                if (*it != "START" && *it != "END") {
                    ...
                    if (head_index == 0) {
                        ...
                        int distance = i - head_index;
                        //count_iter = distance_count.find(distance);
                       // if (count_iter == distance_count.end()) {
                            //distance_count[distance] = 1;
                        //} else {
                           // distance_count[distance] = distance_count[distance] + 1;
                        //}
                        output << i - head_index << '\n';
                    }
                }
