...
int main() {
    ...
    std::ofstream output_distance_count("only_distance_count.txt");
        ...
        while (getline (myfile,line)) {
            ...
            for (std::vector<std::string>::iterator it = curr_sentence.begin(); it != curr_sentence.end(); it++) {
                ...
                if (*it != "START" && *it != "END") {
                    ...
                    count_iter = distance_count.find(i - head_index);
                    if (count_iter == distance_count.end()) {
                        distance_count[i - head_index] = 1;
                    } else {
                         distance_count[i - head_index] = distance_count[i - head_index] + 1;
                    }
                    output << i - head_index << '\n';
                }
            }
