...
int main() {
	...
	std::vector<int> head;
	std::ifstream myfile ("english_wsj_dev_edited.conll");
	std::ofstream output_file("output_file_2.txt");
	int result = -1;
	      ...
	      if (line.size() > 1) { //if the line is not empty
	      		...
	      		if (result == 1) { //this indicates the beginning of a new sentence
	      			...
	      			if (first == true) {
	      				...
	      				output_file << *(pos_tags.begin());
	      				//write the vector into an external file
		      			for (std::vector<std::string>::iterator it = (pos_tags.begin()+1); it != pos_tags.end(); it++) {
		      				output_file << ' ' << *it ;
		      			}
		      			output_file << '\t';
		      			output_file << *(head.begin());
		      			for (std::vector<int>::iterator it = (head.begin()+1); it != head.end(); it++) {
		      				output_file << ' ' << *it;
		      			}
		      			output_file << '\n';
	      			}
