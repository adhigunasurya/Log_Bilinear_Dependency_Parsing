...
int main() {
	std::ifstream counts("counts.txt");
	...
	fine_pos_tags.close();
	//get the frequencies
	while (getline(counts, line)) {
		std::vector<std::string> temp = split(line, ' ');
		if (StringToNumber<double> (temp[2]) > postag_frequency_map.at(temp[0])) {
			postag_frequency_map[temp[0]] = StringToNumber<double> (temp[2]);
			postag_postag_map[temp[0]] = temp[1];
		}
	}
	std::ofstream check_result ("check_result_manual_count.txt");
