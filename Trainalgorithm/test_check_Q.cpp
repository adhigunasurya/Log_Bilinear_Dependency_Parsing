...
int main()
{
	...
	std::ofstream gradient_check_Q("gradient_check_Q_big.txt");
	Eigen::MatrixXd curr_dQ = Eigen::MatrixXd::Zero(FEATURE_SIZE, NO_OF_POSTAGS); //DELETE THIS LATER
	int no_of_iter = 0;
			...
			while (getline(pairs, line)) {
				if (line.size() > 1) {
					...
					if (no_of_iter % 1000 == 0) {
							...
							gradient_check_Q <<  "current data point = " << no_of_iter <<std::endl;
								for (int j = 0; j < Q.rows(); j++) {
									for (int k = 0; k < Q.cols(); k++) {
										...
										result_1 += temp.transpose() * Q_copy.col(arr_temp[0]).matrix();
										for (int l = 0; l < Q.cols(); l++) {
											temp_double += exp(temp.transpose() * Q_copy.col(l).matrix());
										}
										...
										result_2 += temp.transpose() * Q_copy.col(arr_temp[0]).matrix();
										for (int l = 0; l < Q.cols(); l++) {
											temp_double += exp(temp.transpose() * Q_copy.col(l).matrix());
										}
