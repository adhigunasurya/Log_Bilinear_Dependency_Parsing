...
int main()
{
	...
	std::ofstream parameters ("parameters_each_epoch_small_trial_1_check_bias.txt");
	...
	int no_of_iter = 0;
	std::ofstream gradient_check_bias("gradient_check_bias_85000.txt");
	Eigen::VectorXd curr_bias = Eigen::VectorXd::Zero(NO_OF_POSTAGS_WITHOUT_DUMMY * NO_OF_DISTANCE); //DELETE THIS LATER
		...
		if (!pairs.is_open()) {
			...
			while (getline(pairs, line)) {
				if (line.size() > 1) {
					...
					if (no_of_iter == 85000) {
						curr_bias = (temp_dbias - temp_e.matrix() / normalising_constant);
					}
					...
					if (no_of_iter == 85000) {
							...
							gradient_check_bias <<  "current data point = " << no_of_iter <<std::endl;
							for (int j = 0; j < word_bias_pair.rows(); j++) {
									Eigen::ArrayXXd word_bias_pair_copy = word_bias_pair;
									double result_1 = 0.0;
									double result_2 = 0.0;
									double temp_double = 0.0;
									gradient_check_bias << curr_bias(j) << " ";
									word_bias_pair_copy(j) += EPSILON;
									for (int iter2 = 0; iter2 < 5; iter2++) {
										temp += (Ci[iter2].matrix() * R.col(arr_temp[iter2+1]).matrix());
									}
									result_1 += temp.transpose() * Q.col(arr_temp[0]).matrix() + word_bias_pair_copy(arr_temp[0]);
									for (int l = 0; l < Q.cols(); l++) {
										temp_double += exp(temp.transpose() * Q.col(l).matrix() + word_bias_pair_copy(l));
									}
									result_1 -= log(temp_double);
									temp_double = 0.0;
									//calculate result_2
									word_bias_pair_copy(j) -= (2 * EPSILON);
									temp.setZero();
									for (int iter2 = 0; iter2 < 5; iter2++) {
										temp += (Ci[iter2].matrix() * R.col(arr_temp[iter2 + 1]).matrix());
									}
									result_2 += temp.transpose() * Q.col(arr_temp[0]).matrix() + word_bias_pair_copy(arr_temp[0]);
									for (int l = 0; l < Q.cols(); l++) {
										temp_double += exp(temp.transpose() * Q.col(l).matrix() + word_bias_pair_copy(l));
									}
									result_2 -= log(temp_double);
									temp.setZero();
									gradient_check_bias << (result_1 - result_2) / (2 * EPSILON) << std::endl;
								}
						}
					...
					if (no_of_iter % BATCH_SIZE == 0) {
						...
						int j = -1;
						word_bias_pair += curr_learning_rate * dbias.array();
