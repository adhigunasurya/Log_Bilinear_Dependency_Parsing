...
#define LEARNING_RATE 0.1 //LEARNING RATE USED FOR PREVIOUS EXPERIMENTS: 0.06
#endif


//no need for learning decay, use adagrad
#ifndef LEARNING_DECAY
#define LEARNING_DECAY 0.00002
#endif 
...
int main()
{
	...
	std::ofstream parameters ("parameters_each_epoch_small_trial_1_test.txt");
	...
	std::vector<double> sum_of_gradients(8, 0.0); //order: C1, C2, ..., C5, R, Q, bias
	/*
	...
	std::ofstream curr_R("curr_R.txt");
	std::ofstream curr_Q("curr_Q.txt");
	std::ofstream curr_bias("curr_bias.txt");
	std::ofstream curr_C("curr_C.txt");
	std::ofstream normalisation_constant("normalisation_constant.txt");
	//ONLY FOR DEBUGGING PURPOSES
		...
		if (!pairs.is_open()) {
			...
			while (getline(pairs, line)) {
				if (line.size() > 1) {
					...
					if (no_of_iter == 1 || no_of_iter == 300 || no_of_iter == 600 || no_of_iter == 900 || no_of_iter == 1200 || no_of_iter == 1400 || no_of_iter == 1401) {
						curr_R << "no_of_iter = " << no_of_iter << std::endl;
						curr_Q << "no_of_iter = " << no_of_iter << std::endl;
						curr_bias << "no_of_iter = " << no_of_iter << std::endl;
						curr_C << "no_of_iter = " << no_of_iter << std::endl;
						curr_R << R << std::endl;
						curr_Q << Q << std::endl;
						curr_bias << word_bias_pair << std::endl;
						for (int j = 0; j < 5; j++) {
							curr_C << "matrix C " << j+1 << std::endl;
							curr_C << Ci[j] << std::endl;
						}
					}
					normalisation_constant << normalising_constant << std::endl;
					//FOR DEBUGGING PURPOSES ONLY
