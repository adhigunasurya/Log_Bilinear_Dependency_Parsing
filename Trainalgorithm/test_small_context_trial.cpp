...
#define LEARNING_RATE 0.001 //LEARNING RATE USED FOR PREVIOUS EXPERIMENTS: 0.06
#endif
...
int main()
{
	...
	std::normal_distribution<double> distr(0.0, 0.05);
	...
	std::ofstream check_probability("check_probability.txt");
	fine_pos_tags.open("fine_pos_tags.txt");
	...
	for (int i = 0; i < NO_OF_EPOCH; i++) { 
		if (!pairs.is_open()) {
			...
			while (getline(pairs, line)) {
				if (line.size() > 1) {
					...
					int arr_temp[6] = {mapping.at(temp[0]), mapping.at(temp[1]), mapping.at(temp[2]), mapping.at(temp[3]), mapping.at(temp[4]), mapping.at(temp[5])};
				...
				for (int j = 0; j < temp.size(); j++) { //j = head
					for (int k = 0; k < temp.size(); k++) { // k = modifier
						if (temp[j] != "START" && temp[j] != "END" && temp[k] != "START" && temp[k] != "END" && j != k) { //neither the head nor the modifier may be start or end
							int arr_temp[6] = {mapping.at(temp[k]), mapping.at(temp[j]), mapping.at(temp[k-1]), mapping.at(temp[k+1]), mapping.at(temp[j-1]), mapping.at(temp[j+1])};
							for (int l = 0; l < 1; l++) {
		...
		//print out the probability of each pos tag given its head
		check_probability << "Current epoch = " << i+1 << std::endl;
 		for (int j = 0; j < fine_pos_tags_array.size(); j++) { //head
			for (int k = 0; k < fine_pos_tags_array.size(); k++) { //modifier
				check_probability << fine_pos_tags_array[k] << " | " << fine_pos_tags_array[j] << " = ";
				double log_prob = (Ci[0].matrix() * R.col(mapping.at(fine_pos_tags_array[j])).matrix()).transpose() * Q.col(mapping.at(fine_pos_tags_array[k])).matrix();
				double normalising_constant = 0.0;
				for (int l = 0; l < Q.cols(); l++) {
					normalising_constant += exp((Ci[0].matrix() * R.col(mapping.at(fine_pos_tags_array[j])).matrix()).transpose() * Q.col(l).matrix());
				}
				check_probability << log_prob - log(normalising_constant) << std::endl;
			}
		}
		//end of printing out probability
		pairs.open("real_pairs_medium.txt");
	} 
	pairs.close();
	parameters.close();
