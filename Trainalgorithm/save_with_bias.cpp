...
#define LEARNING_RATE 0.0025 //LEARNING RATE USED FOR PREVIOUS EXPERIMENTS: 0.06
#endif


//no need for learning decay, use adagrad
#ifndef LEARNING_DECAY
#define LEARNING_DECAY 0.0000225
#endif 

#ifndef NO_OF_EPOCH
#define NO_OF_EPOCH 13
#endif
...
int main()
{
	...
	if (!big_pos_tag_mapping.is_open() || !small_pos_tag_mapping.is_open()) {
		std::cout << "The mapping file fails to open" << std::endl;
	} else {
		for (std::map<std::string, int>::iterator it = big_pos_tag_map.begin(); it != big_pos_tag_map.end(); it++) {
	...
	std::ofstream parameters ("parameters_each_epoch_small_trial_8.txt");
			...
			while (getline(pairs, line)) {
				if (line.size() > 1) {
					...
					for (int iter = 0; iter < Q.cols(); iter++) {
						double temp = exp((tempCr.transpose() * Q.col(iter).matrix()) + word_bias_pair(iter));//dot product
						temp_e(iter) = temp; 
