...
int main()
{
	...
	std::ofstream parameters ("parameters_each_epoch_small_trial_6_check_Q.txt");
	...
	int no_of_iter = 0;
	Eigen::MatrixXd curr_dQ = Eigen::MatrixXd::Zero(FEATURE_SIZE, NO_OF_POSTAGS_WITHOUT_DUMMY * NO_OF_DISTANCE); //DELETE THIS LATER
		...
		if (!pairs.is_open()) {
			...
			while (getline(pairs, line)) {
				if (line.size() > 1) {
					...
					if (no_of_iter == 85000) {
						curr_dQ = (temp_Q_Data - temp_Q_Model);
					}
					...
					if (no_of_iter == 85000) {
						std::ofstream gradient_check_Q("gradient_check_Q2_85000.txt");
						if (!gradient_check_Q.is_open()) {
							std::cout << "Fail to open gradient_check_Q2 file" << std::endl;
						} else {
							Eigen::VectorXd temp = Eigen::VectorXd::Zero(FEATURE_SIZE);
							for (int j = 0; j < Q.rows(); j++) {
								for (int k = 0; k < Q.cols(); k++) {
									Eigen::ArrayXXd Q_copy = Q;
									double result_1 = 0.0;
									double result_2 = 0.0;
									double temp_double = 0.0;
									gradient_check_Q << curr_dQ(j, k) << " ";
									Q_copy(j, k) += EPSILON;
									for (int iter2 = 0; iter2 < 5; iter2++) {
										temp += (Ci[iter2].matrix() * R.col(arr_temp[iter2+1]).matrix());
									}
									result_1 += temp.transpose() * Q_copy.col(arr_temp[0]).matrix() + word_bias_pair(arr_temp[0]);
									for (int l = 0; l < Q.cols(); l++) {
										temp_double += exp(temp.transpose() * Q_copy.col(l).matrix() + word_bias_pair(l));
									}
									result_1 -= log(temp_double);
									temp_double = 0.0;
									//calculate result_2
									Q_copy(j, k) -= (2 * EPSILON);
									temp.setZero();
									for (int iter2 = 0; iter2 < 5; iter2++) {
										temp += (Ci[iter2].matrix() * R.col(arr_temp[iter2 + 1]).matrix());
									}
									result_2 += temp.transpose() * Q_copy.col(arr_temp[0]).matrix() + word_bias_pair(arr_temp[0]);
									for (int l = 0; l < Q.cols(); l++) {
										temp_double += exp(temp.transpose() * Q_copy.col(l).matrix() + word_bias_pair(l));
									}
