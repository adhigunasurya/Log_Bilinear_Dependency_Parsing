...
double get_validation_accuracy(std::vector<Eigen::ArrayXXd> Ci, Eigen::ArrayXXd R, std::string validation_input_file, std::string validation_gold_standard, std::map<std::string, int> postag_mapping, std::map<std::string, int> big_postag_mapping, Eigen::ArrayXd bv) {
	std::ifstream input_file(validation_input_file);
	std::ifstream gold_standard(validation_gold_standard);
	if (!input_file.is_open() || !gold_standard.is_open()) {
		std::cout << "output file fails to open" << std::endl;
	} else {
		std::string line;
		std::vector< std::vector<std::string> > arr_of_gold_standard;
		int i = -1;
		//get the gold standards from the validation_gold_standard file
		while (getline(gold_standard, line)) {
			if (line.size() <= 1) {
				std::vector<std::string> temp;
				arr_of_gold_standard.push_back(temp);
				i++;
			} else {
				arr_of_gold_standard[i].push_back(line);
			}
		}
		...
		while (getline(input_file, line)) {
			...
			for (int i = 0; i < sentence.size(); i++) {
				for (int j = 0; j < sentence.size(); j++) {
					if (sentence[i] != "START" && sentence[i] != "END" && sentence[j] != "START" && sentence[j] != "END") {
						//create pairs
						...
						if (postag_mapping.find(sentence[j]) != postag_mapping.end()) {
							vi[0](postag_mapping[sentence[j]]) = 1.0;
						} else {
							std::cout << "postag not mapped" << std::endl;
						}
						if (postag_mapping.find(sentence[j]) != postag_mapping.end()) {
							vi[0](postag_mapping[sentence[j]]) = 1.0;
						} else {
			...
			while (getline(pairs, line)) {
				if (line.size() > 1) {
					...
					for (int j = 0; j < 5; j++) {
						...
						//look at more efficient implementations to reset the matrix
