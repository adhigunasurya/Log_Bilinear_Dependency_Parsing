		...
		if (!pairs.is_open()) {
			...
			while (getline(pairs, line)) {
				if (line.size() > 1) {
					...
					if (no_of_iter == 1) {
						std::ofstream gradient_check_C1("gradient_check_C1.txt");
						if (!gradient_check_C1.is_open()) {
							std::cout << "Fail to open gradient_check_C1 file" << std::endl;
						} else {
							Eigen::VectorXd temp = Eigen::VectorXd::Zero(FEATURE_SIZE);
							for (int j = 0; j < Ci[0].rows(); j++) {
								for (int k = 0; k < Ci[0].cols(); k++) {
									Eigen::ArrayXXd C1_copy = Ci[0];
									double result_1 = 0.0;
									double result_2 = 0.0;
									double temp_double = 0.0;
									gradient_check_C1 << dCi[0](j, k) << " ";
									C1_copy(j, k) += EPSILON;
									for (int iter2 = 0; iter2 < 5; iter2++) {
